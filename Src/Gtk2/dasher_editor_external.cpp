...
static void dasher_editor_external_handle_font(DasherEditor *pSelf, const gchar *szFont);
...
static void dasher_editor_external_setup_actions(DasherEditor *pSelf);
static void dasher_editor_external_add_action(DasherEditor *pSelf, DasherAction *pNewAction);
static EditorAction *dasher_editor_external_get_action_by_id(DasherEditor *pSelf, int iID);
static void dasher_editor_external_rebuild_action_pane(DasherEditor *pSelf);
static void dasher_editor_external_display_message(DasherEditor *pSelf, DasherMessageInfo *pMessageInfo);
static void dasher_editor_external_check_activity(DasherEditor *pSelf, EditorAction *pAction);
static void dasher_editor_external_action_save_state(DasherEditor *pSelf, EditorAction *pAction);
...
#ifdef GNOME_LIBS
static void dasher_editor_external_vfs_print_error(DasherEditor *pSelf, GnomeVFSResult * result, const char *myfilename);
static gboolean dasher_editor_external_gnome_vfs_open_file(DasherEditor *pSelf, const char *filename, gchar ** buffer, unsigned long long *size);
static gboolean dasher_editor_external_gnome_vfs_save_file(DasherEditor *pSelf, const char *filename, gchar * buffer, unsigned long long length, bool append);
#else
static gboolean dasher_editor_external_unix_vfs_open_file(DasherEditor *pSelf, const char *filename, gchar ** buffer, unsigned long long *size);
static gboolean dasher_editor_external_unix_vfs_save_file(DasherEditor *pSelf, const char *filename, gchar * buffer, unsigned long long length, bool append);
#endif
...
static void dasher_editor_external_generate_filename(DasherEditor *pSelf);
static void dasher_editor_external_open(DasherEditor *pSelf, const gchar *szFilename);
static bool dasher_editor_external_save_as(DasherEditor *pSelf, const gchar *szFilename, bool bAppend);
static void dasher_editor_external_create_buffer(DasherEditor *pSelf);
static void dasher_editor_external_clear(DasherEditor *pSelf, gboolean bStore);
static void dasher_editor_external_clipboard(DasherEditor *pSelf, clipboard_action act);
...
void dasher_editor_external_handle_stop(DasherEditor *pSelf);
void dasher_editor_external_handle_start(DasherEditor *pSelf);
...
void dasher_editor_external_action_button(DasherEditor *pSelf, DasherAction *pAction);
void dasher_editor_external_actions_start(DasherEditor *pSelf);
bool dasher_editor_external_actions_more(DasherEditor *pSelf);
void dasher_editor_external_actions_get_next(DasherEditor *pSelf, const gchar **szName, gint *iID, gboolean *bShow, gboolean *bControl, gboolean *bAuto);
void dasher_editor_external_action_set_show(DasherEditor *pSelf, int iActionID, bool bValue);
void dasher_editor_external_action_set_control(DasherEditor *pSelf, int iActionID, bool bValue);
void dasher_editor_external_action_set_auto(DasherEditor *pSelf, int iActionID, bool bValue);
...
// 							G_TYPE_NONE, 0);
  ...
  pParentClass->handle_font = dasher_editor_external_handle_font;
  ...
  pParentClass->handle_stop = dasher_editor_external_handle_stop;
  pParentClass->handle_start = dasher_editor_external_handle_start;
  pParentClass->handle_control = dasher_editor_external_handle_control;
  pParentClass->action_button = dasher_editor_external_action_button;
  pParentClass->actions_start = dasher_editor_external_actions_start;
  pParentClass->actions_more = dasher_editor_external_actions_more;
  pParentClass->actions_get_next = dasher_editor_external_actions_get_next;
  pParentClass->action_set_show = dasher_editor_external_action_set_show;
  pParentClass->action_set_control = dasher_editor_external_action_set_control;
  pParentClass->action_set_auto = dasher_editor_external_action_set_auto;
  pParentClass->grab_focus = dasher_editor_external_grab_focus;
  pParentClass->file_changed = dasher_editor_external_file_changed;
  pParentClass->get_filename = dasher_editor_external_get_filename;
...
dasher_editor_external_initialise(DasherEditor *pSelf, DasherAppSettings *pAppSettings, DasherMain *pDasherMain, GladeXML *pGladeXML, const gchar *szFullPath) {
  ...
  dasher_editor_external_setup_actions(pSelf);

  // TODO: see note in command_new method
  if(szFullPath)
    dasher_editor_external_open(pSelf, szFullPath);
  else {
    dasher_editor_external_generate_filename(pSelf);
    dasher_editor_external_clear(pSelf, false);
  }
}

static void 
dasher_editor_external_clipboard(DasherEditor *pSelf, clipboard_action act) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
  ...
  case CLIPBOARD_SELECTALL:
    dasher_editor_external_select_all(pSelf);
    break;
...
void 
dasher_editor_external_handle_stop(DasherEditor *pSelf) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
void 
dasher_editor_external_handle_start(DasherEditor *pSelf) {
...
dasher_editor_external_handle_control(DasherEditor *pSelf, int iNodeID) {
  ...
  if(iNodeID == Dasher::CControlManager::CTL_USER + 1)
    dasher_editor_external_clear(pSelf, false); // Clear node is a special case (it shouldn't be)
  else {
    EditorAction *pCurrentAction = pPrivate->pActionRing;
    bool bStarted = false;
	...
	//	dasher_editor_external_clear(pSelf, true); 
      }
      pCurrentAction = pCurrentAction->pNext;
    }
  }
...
void 
dasher_editor_external_action_button(DasherEditor *pSelf, DasherAction *pAction) {
  if(pAction) {
...
static void 
dasher_editor_external_clear(DasherEditor *pSelf, gboolean bStore) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
void 
dasher_editor_external_actions_start(DasherEditor *pSelf) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
bool 
dasher_editor_external_actions_more(DasherEditor *pSelf) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
void 
dasher_editor_external_actions_get_next(DasherEditor *pSelf, const gchar **szName, gint *iID, gboolean *bShow, gboolean *bControl, gboolean *bAuto) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
void 
dasher_editor_external_action_set_show(DasherEditor *pSelf, int iActionID, bool bValue) {
  EditorAction *pAction;
  pAction = dasher_editor_external_get_action_by_id(pSelf, iActionID);

  if(pAction) {
    pAction->bShow = bValue;
    dasher_editor_external_check_activity(pSelf, pAction);
    dasher_editor_external_rebuild_action_pane(pSelf);

    dasher_editor_external_action_save_state(pSelf, pAction);
  }
}

void 
dasher_editor_external_action_set_control(DasherEditor *pSelf, int iActionID, bool bValue) {
...
void 
dasher_editor_external_action_set_auto(DasherEditor *pSelf, int iActionID, bool bValue) { 
EditorAction *pAction;
  pAction = dasher_editor_external_get_action_by_id(pSelf, iActionID);
...
static void 
dasher_editor_external_generate_filename(DasherEditor *pSelf) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
  ...
  dasher_editor_external_set_filename(pSelf, szNewFilename);

  g_free(szNewFilename);
}

static void 
dasher_editor_external_open(DasherEditor *pSelf, const gchar *szFilename) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
#ifdef GNOME_LIBS
  if(!dasher_editor_external_gnome_vfs_open_file(pSelf, szFilename, &buffer, &size)) {
    return;
  }
#else
  if(!dasher_editor_external_unix_vfs_open_file(pSelf, szFilename, &buffer, &size)) {
    return;
  }
#endif
...
// 							myfilename);
  ...
  dasher_editor_external_set_filename(pSelf, szFilename);
}

static bool 
dasher_editor_external_save_as(DasherEditor *pSelf, const gchar *szFilename, bool bAppend) { 
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
#ifdef GNOME_LIBS
  if(!dasher_editor_external_gnome_vfs_save_file(pSelf, szFilename, outbuffer, bytes_written, bAppend)) {
    return false;
  }
#else
  if(!dasher_editor_external_unix_vfs_save_file(pSelf, szFilename, outbuffer, bytes_written, bAppend)) {
    return false;
  }
#endif
...
// void 
// dasher_editor_external_start_tutorial(DasherEditor *pSelf) {
//    DasherEditorExternalPrivate *pPrivate = (DasherEditorExternalPrivate *)(pSelf->private_data);
...
dasher_editor_external_command(DasherEditor *pSelf, const gchar *szCommand) {
  ...
  if(!strcmp(szCommand, "new")) { //select_new_file
    dasher_editor_external_command_new(pSelf);
    return TRUE;
  }
  
  if(!strcmp(szCommand, "open")) { //select open file
    dasher_editor_external_command_open(pSelf);
    return TRUE;
  }
  
  if(!strcmp(szCommand, "save")) { //save_file
    dasher_editor_external_command_save(pSelf, FALSE, FALSE);
    return TRUE;
  }

  if(!strcmp(szCommand, "saveas")) { // select_save_file_as
    dasher_editor_external_command_save(pSelf, TRUE, FALSE);
    return TRUE;
  }

  if(!strcmp(szCommand, "append")) { // select_append_file
    dasher_editor_external_command_save(pSelf, TRUE, TRUE);
    return TRUE;
  }

  if(!strcmp(szCommand, "cut")) { // clipboard_cut
    dasher_editor_external_clipboard(pSelf, CLIPBOARD_CUT);
    return TRUE;
  }

  if(!strcmp(szCommand, "copy")) { // clipboard_copy
    dasher_editor_external_clipboard(pSelf, CLIPBOARD_COPY);
    return TRUE;
  }

  if(!strcmp(szCommand, "copyall")) { // clipboard_copyall 
    dasher_editor_external_clipboard(pSelf, CLIPBOARD_COPYALL);
    return TRUE;
  }

  if(!strcmp(szCommand, "paste")) { // clipboard_paste
    dasher_editor_external_clipboard(pSelf, CLIPBOARD_PASTE);
    return TRUE;
  }
 
  // TODO: This isn't actually accessible from anywhere
  if(!strcmp(szCommand, "selectall")) { // clipboard_paste
    dasher_editor_external_clipboard(pSelf, CLIPBOARD_SELECTALL);
    return TRUE;
  }
...
void 
dasher_editor_external_handle_font(DasherEditor *pSelf, const gchar *szFont) {
  if(strcmp(szFont, "")) {
    DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
gboolean 
dasher_editor_external_file_changed(DasherEditor *pSelf) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
  return pPrivate->bFileModified;
}

const gchar *
dasher_editor_external_get_filename(DasherEditor *pSelf) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
/* Private methods */
static void 
dasher_editor_external_select_all(DasherEditor *pSelf) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
  GtkTextIter *start, *end;
...
static void 
dasher_editor_external_setup_actions(DasherEditor *pSelf) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
#ifdef GNOME_SPEECH
  dasher_editor_external_add_action(pSelf, DASHER_ACTION(dasher_action_speech_new()));
#endif

  dasher_editor_external_add_action(pSelf, DASHER_ACTION(dasher_action_keyboard_new(pPrivate->pExternalBuffer)));

#ifdef WITH_MAEMO
  dasher_editor_external_add_action(pSelf, DASHER_ACTION(dasher_action_keyboard_maemo_new()));
#else
  //  dasher_editor_external_add_action(pSelf, DASHER_ACTION(dasher_action_copy_new(pSelf)));
  ...
  if(pDirectory) {
    while((szFilename = g_dir_read_name(pDirectory))) {
      dasher_editor_external_add_action(pSelf, DASHER_ACTION(dasher_action_script_new(szUserScriptDir, szFilename)));
    }
  ...
  if(pDirectory) {
    while((szFilename = g_dir_read_name(pDirectory))) {
      dasher_editor_external_add_action(pSelf, DASHER_ACTION(dasher_action_script_new(szSystemScriptDir, szFilename)));
    }
...
static void 
dasher_editor_external_add_action(DasherEditor *pSelf, DasherAction *pNewAction) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
  ...
  dasher_editor_external_check_activity(pSelf, pNewEditorAction);
...
static EditorAction *
dasher_editor_external_get_action_by_id(DasherEditor *pSelf, int iID){
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
static void 
dasher_editor_external_rebuild_action_pane(DasherEditor *pSelf) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
static void 
dasher_editor_external_display_message(DasherEditor *pSelf, DasherMessageInfo *pMessageInfo) {
...
static void 
dasher_editor_external_check_activity(DasherEditor *pSelf, EditorAction *pAction) {
...
static void 
dasher_editor_external_action_save_state(DasherEditor *pSelf, EditorAction *pAction) {
  gchar szRegistryName[256];
  strncpy(szRegistryName, "Action_", 256);
...
static void 
dasher_editor_external_command_new(DasherEditor *pSelf) {
  dasher_editor_external_new_buffer(pSelf, NULL);
}

static void 
dasher_editor_external_command_open(DasherEditor *pSelf) { 
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
#endif
    dasher_editor_external_new_buffer(pSelf, filename);
    g_free(filename);
  }
...
static void 
dasher_editor_external_command_save(DasherEditor *pSelf, gboolean bPrompt, gboolean bAppend) { 
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
  ...
  dasher_editor_external_save_as(pSelf, szFilename, bAppend);
  g_free(szFilename);
}

#ifdef GNOME_LIBS
static void 
dasher_editor_external_vfs_print_error(DasherEditor *pSelf, GnomeVFSResult *result, const char *myfilename) {
...
static gboolean 
dasher_editor_external_gnome_vfs_open_file(DasherEditor *pSelf, const char *myfilename, gchar **buffer, unsigned long long *size) {
...
//     }
  ...
  if(result != GNOME_VFS_OK) {
    dasher_editor_external_vfs_print_error(pSelf, &result, myfilename);
    g_free(uri);
    return FALSE;
  }
...
static gboolean 
dasher_editor_external_gnome_vfs_save_file(DasherEditor *pSelf, const char *myfilename, gchar *buffer, unsigned long long length, bool append) {
...
static gboolean 
dasher_editor_external_unix_vfs_open_file(DasherEditor *pSelf, const char *myfilename, gchar **buffer, unsigned long long *size) {
  GtkWidget *error_dialog;
...
static gboolean 
dasher_editor_external_unix_vfs_save_file(DasherEditor *pSelf, const char *myfilename, gchar *buffer, unsigned long long length, bool append) {
  int opened = 1;
  GtkWidget *error_dialog;
...
static void 
dasher_editor_external_set_filename(DasherEditor *pSelf, const gchar *szFilename) {
  DasherEditorExternalPrivate *pPrivate = DASHER_EDITOR_EXTERNAL_GET_PRIVATE(pSelf);
...
dasher_editor_external_new_buffer(DasherEditor *pSelf, const gchar *szFilename) {
  ...
  if(szFilename) {
    dasher_editor_external_open(pSelf, szFilename);
  }
  else {
    dasher_editor_external_generate_filename(pSelf);
    dasher_editor_external_clear(pSelf, false);
...
action_button_callback(GtkWidget *pWidget, gpointer pUserData) { 
  ...
  dasher_editor_external_action_button((DasherEditor *)pPointers[0], (DasherAction *)pPointers[1]);
}
