...
#include "JapneseLanguageModel.h"
...
CJapaneseLanguageModel::CJapaneseLanguageModel(const CSymbolAlphabet& SymbolAlphabet, CLanguageModelParams *_params)
...
void CJapaneseLanguageModel::GetProbs( Context context,vector<unsigned int> &probs, int norm) const
{
	...
	for( i=0; i<ppmcontext->history.size(); i++ ){
		if( ppmcontext->history[i] == GetEndConversionSymbol() ){
			...
			// We found the conversion symbol!
