...
namespace Dasher 
{
	...
	{
	public:
		CJapaneseLanguageModel(const CSymbolAlphabet& alph, CLanguageModelParams *_params);
	...
	private:
		...
		{
		public:
			CPPMContext(CPPMContext const &input) {head = input.head;	order= input.order;	history=input.history;	}
		...
		symbol GetStartConversionSymbol() const {return SymbolAlphabet().GetAlphabetPointer()->GetStartConversionSymbol();}		//Added 23 June 2005
		symbol GetEndConversionSymbol() const {return SymbolAlphabet().GetAlphabetPointer()->GetEndConversionSymbol();}	//Added 7 July 2005
		
		const std::string& GetText(symbol i) const {return SymbolAlphabet().GetAlphabetPointer()->GetText(i);}     // return string for i'th symbol
		const std::string& GetDisplayText(symbol i) const {return SymbolAlphabet().GetAlphabetPointer()->GetDisplayText(i);}     // return string for i'th symbol
